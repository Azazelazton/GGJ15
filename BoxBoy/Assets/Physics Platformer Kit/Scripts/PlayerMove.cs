...
public class PlayerMove : Photon.MonoBehaviour 
{
	...
	void FixedUpdate() 
	{
		...
		if(animator)
		{
			animator.SetFloat("DistanceToTarget", direction != Vector3.zero? characterMotor.DistanceToTarget: 0 );
	...
	void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
	{
		if (stream.isWriting)
		{
			stream.SendNext(curAccel);
			stream.SendNext(curDecel);
			stream.SendNext(curRotateSpeed);
		...
		{
			curAccel = (float) stream.ReceiveNext();
			curDecel = (float) stream.ReceiveNext();
			curRotateSpeed = (float) stream.ReceiveNext();
